      ...
      <section className="w-full pt-6 md:pt-8 pb-4 px-gutter-mobile md:px-margin-desktop max-w-[1440px] mx-auto text-center flex flex-col items-center justify-center">
        <FadeIn>
          ...
          <p className="font-body-lg text-body-lg text-on-surface-variant max-w-2xl mx-auto">
            Discover our curated selection of premium handbags, designed for the modern woman who appreciates timeless elegance and impeccable craftsmanship.
          </p>
        </FadeIn>
      </section>
      ...
      <section className="w-full px-gutter-mobile md:px-margin-desktop max-w-[1440px] mx-auto mb-12 sticky top-20 z-40 bg-background/95 backdrop-blur-md py-4 border-b border-surface-container">
        <div className="flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex items-center gap-3 overflow-x-auto w-full md:w-auto pb-2 md:pb-0">
            <button
              onClick={() => { setActiveFilter("all"); setSortBy("Newest"); }}
              className={`px-6 py-2 rounded-full font-label-sm text-label-sm uppercase tracking-widest whitespace-nowrap border transition-colors duration-300 ${
                activeFilter === "all"
                  ? "bg-tertiary-fixed text-primary border-transparent"
                  : "bg-surface-container-low text-on-surface-variant hover:text-primary border-transparent hover:border-muted-gold"
              }`}
            >
              All Bags
            </button>
